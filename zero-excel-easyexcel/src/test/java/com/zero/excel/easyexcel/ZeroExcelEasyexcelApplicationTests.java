package com.zero.excel.easyexcel;
...
import com.zero.excel.easyexcel.common.handler.CommentWriteHandler;
import com.zero.excel.easyexcel.common.handler.CustomCellWriteHandler;
import com.zero.excel.easyexcel.common.handler.CustomSheetWriteHandler;
import com.zero.excel.easyexcel.common.model.DemoData;
import com.zero.excel.easyexcel.common.model.WriteCellDemoData;
import com.zero.excel.easyexcel.common.util.EasyExcelUtils;
...
@Slf4j
@SpringBootTest
class ZeroExcelEasyexcelApplicationTests {
    ...
    public void writeCellDataWrite() {
        String fileName = EasyExcelUtils.getPath() + "writeCellDataWrite" + System.currentTimeMillis() + ".xlsx";
    ...
    public void customHandlerWrite() {
        String fileName = EasyExcelUtils.getPath() + "customHandlerWrite" + System.currentTimeMillis() + ".xlsx";
        // 这里 需要指定写用哪个class去写，然后写到第一个sheet，名字为模板 然后文件流会自动关闭
        EasyExcel.write(fileName, DemoData.class).registerWriteHandler(new CustomSheetWriteHandler(list.size()))
                .registerWriteHandler(new CustomCellWriteHandler()).sheet("模板").doWrite(list);
    }
    ...
    public void commentWrite() {
        String fileName = EasyExcelUtils.getPath() + "commentWrite" + System.currentTimeMillis() + ".xlsx";
        // 这里 需要指定写用哪个class去写，然后写到第一个sheet，名字为模板 然后文件流会自动关闭
